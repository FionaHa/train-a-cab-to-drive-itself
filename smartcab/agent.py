...
class LearningAgent(Agent):
    ...
    def get_maxQ(self, state):
        ...
        for key_state in self.Q.keys():
            if key_state == state:
                maxQ = max(self.Q[key_state].values())
                ...
                if key_state == state:
                    for key_action in self.Q[state].keys():
                        if key_action == action:
                            ...
                            alpha * (reward + self.gamma * self.get_maxQ(state) - self.Q[state][action]) 
...
def run():
    ...
    # __init__() takes at least 2 arguments error when running agent = env.create_agent(LearningAgent(learning=True)) 
